...
评测运行方式见 [评测说明](evals/README.md)：它会对真实接口记录执行结构化评分，并输出 JSON、Markdown、质量指标、服务端/端到端/评测耗时的 P50/P95 和评测 token 用量。回归套件覆盖上传和解析、嵌入缓存、用户隔离检索、RAG 引用与安全拒答、工具调用和超时、管理员权限、审计脱敏及评测报告格式。最终容器验收同时检查健康状态、迁移一致性和 OpenAPI 路由。
